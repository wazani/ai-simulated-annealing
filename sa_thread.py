...
def run(user_config, message_queue, terminate_event):
    ...
    try:
        config = Config(
            ...
            is_objective=bool(user_config["is_objective"]),
        )
        allItems = []
        for item in user_config["items"]:
            allItems.append(
                Item(item["label"], int(item["weight"]), int(item["value"]))
            )
        ...
        message_queue.put(
            (-1, -1, "Error: Invalid input. Please enter valid integers.")
        )
        ...
        message_queue.put(
            (
                -1,
                -1,
                "An unexpected error occurred. Please make sure you entered valid inputs. see logs for more details",
            )
        )
