        ...
        eel.progress(
            {
                "i": queue_msg[0],
                "status": queue_msg[1],
                "current_temp": queue_msg[4],
                "current_state": current_state.bag.json(),
                "best_state": best_state.bag.json(),
            }
        )
    else:
        eel.progress(
            {"i": queue_msg[0], "status": queue_msg[1], "message": queue_msg[2]}
        )
...
def start(user_config):
    ...
    simulated_annealing_process = threading.Thread(
        target=run, args=(user_config, message_queue, terminate_event)
    )
...
if __name__ == "__main__":
    eel.start("index.html", mode="edge", port=0, cmdline_args=["--start-maximized"])
